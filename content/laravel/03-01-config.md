...
part: 01
title: درس ۱: پیکربندی‌ها در لاراول
_index: config
---
...
بنابراین ما اولین گام عملی در پروژه‌ی آپولو هوا کردن خودمان در این کتاب را با تنظیم دیتابیس شروع می‌کنیم و اولین درس تنظیم دیتابیس را به پیکربندی اختصاص می‌دهیم[^c030101].

[^c030101]: امیدوارم این توجیه شما را قانع کرده باشد، اما اگر این طور نیست، این یکی را امتحان کنید: هنگام طراحی نقشه‌ی روند آموزشی کتاب، لازم دیدم که پیکربندی را پیش از دیتابیس بشکافم و جای این موضوع در فصل قبل که به مبانی سیستمی لاراول می‌پرداخت نبود و در عین حال نمی‌توانستم یک فصل مستقل برای پیکربندی در نظر بگیرم. اگر باز هم قانع نشده‌اید، چاره‌ای نیست. همین طور که هست تحمل کنید و به خواندن ادامه دهید.
...
## بازخوانی به کمک تابع config

بازخوانی پیکربندی‌ها در لاراول، با استفاده از تابع config انجام می‌شود. به تابع config، بگویید که دنبال چه گزینه‌ای می‌گردید و آن را بگیرید.
...
![](_images/03-01-02.png)

در مثال بالا، سه اجرا از تابع config را می‌بینید. در اولی به دنبال چیزی بودیم که وجود داشت، و پاسخش را هم گرفتیم. در دومی به دنبال چیزی بودیم که وجود نداشت و null را دریافت کردیم، و در سومی دنبال چیزی بودیم که وجود نداشت، اما مقدار پیش‌فرضی را برای بازگشت انتخاب کرده بودیم و لاراول همان مقدار پیش‌فرض را به ما بازگرداند.
...
اگر می‌خواهید در هنگام اجرای برنامه مقدار یک پیکربندی را تغییر دهید، آرایه‌ی تغییرات را به تابع config ارسال کنید.
	...
	config(['apollo.measuring_unit.weight' => 'g']);

## پیکربندی‌هایی وابسته به محیط

فایل‌های پیکربندی لاراول، چنانچه مرسوم است، تحت نظارت سیستم کنترل نسخه‌ی شما قرار می‌گیرند و در میان تیم برنامه‌نویسی یکسان ظاهر می‌شوند و طبیعتاً به همان نحو که در فاز توسعه ایجاد می‌شوند، در فاز عملیاتی، روی سرور هم مورد استفاده قرار می‌گیرند.

اگر مایلید که هر کدام از اعضای تیم شما تنظیمات مخصوص به خود را در پاره‌ای از موارد داشته باشند، اگر آنچه در فاز اجرا نیاز دارید با فاز توسعه متفاوت است، و اگر به هر دلیل دیگری می‌خواهید تنظیمات خاصی را از دسترس سیستم کنترل نسخه خارج کنید، می‌توانید از قابلیت git ignore یا معادل‌های آن استفاده کنید، اما راهش این نیست!
پیش از این، هنگام نصب لاراول، از فایلی مخصوص به نام .env سخن گفتیم. لاراول این فایل ویژه را برای همین کارها در نظر گرفته تا از دسترس سیستم کنترل نسخه مصون بدارد. پیکربندی‌هایی که اینجا تعریف می‌کنید، فقط برای شماست.
...
[^c030102]: این آخرین نکته‌ای است که می‌نویسم، اما آخرین نکته‌ای نیست که می‌توانید بدانید. نگاه دیگری به عنوان کتاب بیاندازید و برای اطلاعات بیشتر در مورد هر موضوعی که در این کتاب مطرح شده، به مستندات دست اول لاراول مراجعه نمایید.
...
به یاد دارید که مشکلات لاراولی، راه‌حل‌هایی لاراولی نیز دارند.
...
نکته‌ی آخری که در بخش پیش گفتم، را به خاطر دارید؟

سعی کنید در متن برنامه‌های خود از تابع env استفاده نکنید و هر گزینه‌ای که می‌خواهید را از طریق همان تابع config فراخوانی کنید. از تابع env فقط در هنگام تعریف تنظیمات در فایل‌های پیکربندی استفاده کنید و بس.
داستان از این قرار است که پس از کَش شدن پیکربندی‌ها، لاراول دیگر به سراغ فایل .env نمی‌رود و از آن پس، تابع env، همیشه مقدار null را بازمی‌گرداند.
