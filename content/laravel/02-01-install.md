...
part: 01
title: درس ۱:‌ نصب و راه اندازی لاراول
_index: install
---
...
لاراول راهی ساده‌تر و زیباتر نیز، حتی برای همین بسترهای ساده فراهم کرده hsj. کافی است در پوشه‌ی پروژه‌ای که ساخته‌اید، دستور زیر را اجرا کنید.

```bash
php artisan serve
```
با این کار، پروژه‌ی شما روی یکی از پورت‌های محلی دستگاهتان در دسترس قرار می‌گیرد.
...
احتمال زیادی وجود دارد که با هیچ یک از دو راه زیر نتوانسته باشید لاراول را در مرورگر خود ببینید.
...
به نکته‌ای جالب در این کامنت‌ها توجه کنید که نشان از توجه فوق‌العاده‌‌ی خالق لاراول به جزئیات و زیبایی کدها دارند. هر کدام کامنت‌های این بخش، و همین طور هر کدام کامنت‌های سراسر پروژه، از سه خط تشکیل شده‌اند که هر خط، دقیقاً سه کاراکتر از خط بعدی خود بلندتر است. 
...
پارامتر نخست این تابع، نام این تنظیمات در فایلی به نام .env را مشخص کرده‌اند و پارامتر دوم، مقدار پیش‌فرض را بازمی‌گردانند که اگر گزینه‌ای با نام مشخص‌شده در فایل .env نبود یا اصلاً چنین فایلی وجود نداشت، آن را لحاظ کند.
...
کلید رمزنگاری لاراول، در فایل app.php در پوشه‌ی config، با عبارت key مشخص شده است و در فایل .env نیز با کلید APP_KEY تعریف می‌شود.
...
در فایل app.php (که بالاتر هم صحبتش به میان آمد)، گزینه‌ای با عنوان locale وجود دارد که زبان پروژه‌ی شما را تعیین می‌کند. همچنین گزینه‌ی دیگری نیز با عنوان timezone هست که موقعیت زبانی پروژه‌ی شما را در خود نگاه می‌دارد که اگر مایلید زمان رسمی کشور ایران را مورد استفاده قرار دهید، این عبارت را در آن مقداردهی نمایید:

```php
'timezone' => 'Asia/Tehran' ,
```

فایل database.php را نیز باز کنید و داخلش را ببینید. نام دیتابیسی که پروژه‌ی شما با آن کار می‌کند را در این فایل می‌یابید که می‌تواند از طریق env. نیز تعیین شود. 

در فصل بعدی که به سراغ دیتابیس آمدیم، سری هم به این فایل می‌زنیم، اما همین حالا هم می‌توانید دیتابیسی را برای پروژه‌ی آپولوی خودتان بسازید و نام و اطلاعاتش را به این فایل بدهید. 
